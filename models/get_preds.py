...
def get_predictions(input_image):
    ...
    for i in range(len(lIndexB)):
        ...
        cv.imwrite(
            "Segmentation/lines/line{}.jpg".format(i+1), lines[i])
    ...
    char_model = keras.models.load_model('/model_files/best_val_loss_model.h5')
    ...
    for filename in os.listdir(line_dir):
        ...
        for i in range(len(charsB)):
            ...
            if len(chars[i][0]) != 0:
                cv.imwrite(
                    f"Segmentation/characters/char{kth_img}.png", chars[i])
